...
;(function (factory) {
    ...
    EvoCalendar.prototype.addEventList = function (event_data) {
        ...
        if (event_data.status) {
            event_data.class = 'blocked';
            event_data.badge = 'busy';
        }
        markup = '<div class="event-container ' + event_data.class + ' " data-status="dis" role="button" data-event-index="' + (event_data.id) + '">';
        ...
        for (var i = 0; i < 9; i++) { // this loop is for is weeks (rows)
            for (var j = 0; j < _.$label.days.length; j++) { // this loop is for weekdays (cells)
                if (day <= _.monthLength && (i > 0 || j >= _.startingDay)) {
                    ...
                    date_after.setMonth(date_after.getMonth() + 1);

                    /*
                    console.log(day);
                    console.log(new Date(thisDay));
                    console.log(new Date(thisDay) < new Date());

                    */
                    if (new Date(thisDay) < new Date()) {
                        dayClass = "calendar-day before";
                    }

                    if (new Date(thisDay) > date_after) {
                        dayClass = "calendar-day before";
                    }
